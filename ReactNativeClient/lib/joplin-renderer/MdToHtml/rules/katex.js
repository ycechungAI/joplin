	...
	install: function(context) {
		...
		return function(md, options) {
			...
			const katexInline = function(latex) {
				...
				try {
					return `<span class="joplin-editable mceNonEditable"><span class="joplin-source" data-joplin-source-open="$" data-joplin-source-close="$">${latex}</span>${renderToStringWithCache(latex, options)}</span>`;
				} catch (error) {
			...
			const katexBlock = function(latex) {
				...
				try {
					return `<div class="joplin-editable mceNonEditable"><pre class="joplin-source" data-joplin-source-open="$$&#10;" data-joplin-source-close="&#10;$$&#10;">${latex}</pre>${renderToStringWithCache(latex, options)}</div>`;
				} catch (error) {
					console.error('Katex error for:', latex, error);
					return latex;
				}
